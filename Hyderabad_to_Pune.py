...
def response_json_to_required_csv_format(data,directory_for_saving_seat_data_of_particular_route,operator_id,route_id):
    ...
    final_row = pd.DataFrame({
        'seat': ['Travels Name','journey route','Total'],
        'Fares_Amount': ['','',''],
        'Is_Available': ['','',''],
        'occupied': ['','',''],
        'revenue': [f'{travels_name}',f'{fromcity}-{tocity}',final_revenue]
    })
 ...
 #   formatted_datetime = current_datetime.strftime("%I.%M%p,%d-%m-%Y")
    ...
    now=datetime.datetime.now()    
    now = now.strftime("%I.%M%p,%d-%m-%Y") 
    file_name_format = f"{operator_id}_{route_id}_{now}.csv"
