...
#input for cell : csv file contining b=different buses
now=datetime.datetime.now()
now = now.strftime("%I%p,%d-%m-%Y")
filename = f"buses_on_{next_day}_at_{now}.csv"
...
def response_json_to_required_csv_format(data,directory_for_saving_seat_data_of_particular_route,operator_id,route_id):
    ...
    now = now.strftime("%I%p") 
    file_name_format = f"{travels_name}_{operator_id}_{route_id}_{fromcity}_{tocity}_seat_data_on_{formatted_datetime}_at_{now}.csv"
...
def process_single_route(file_path):
    ...
    df = df[:-2]
    output_directory = os.path.join(output_dir, os.path.splitext(os.path.basename(file_path))[0])
